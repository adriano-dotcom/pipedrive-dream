...
export function useDealFiles(dealId: string) {
  ...
  const uploadFileMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const filePath = `${dealId}/${user.id}/${Date.now()}_${file.name}`;
      ...
      // 2. Save metadata
      const { error: metaError } = await supabase.from('deal_files').insert({
        deal_id: dealId,
        file_name: file.name,
        file_path: filePath,
        file_size: file.size,
      ...
      if (metaError) {
        ...
        await supabase.storage.from('deal-files').remove([filePath]);
        throw metaError;
      }
