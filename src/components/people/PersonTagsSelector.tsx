...
export function PersonTagsSelector({ 
  ...
  const updateTagMutation = useUpdatePersonTag();
  const deleteTagMutation = useDeletePersonTag();
  ...
  const handleStartEdit = (tag: PersonTag) => {
    setEditingTag(tag);
      ...
      <div className="max-h-[250px] overflow-y-auto space-y-1">
        {isLoading ? (
          ...
          allTags.map((tag) => (
            ...
            >
              <PersonTagBadge name={tag.name} color={tag.color} />
  ...
  return (
    <>
      ...
      <DeleteConfirmDialog
        ...
        description={`Tem certeza que deseja excluir a etiqueta "${tagToDelete?.name}"? Ela será removida de todas as pessoas que a possuem. Esta ação não pode ser desfeita.`}
