...
} from '@/hooks/useOrganizationTags';
import { toast } from 'sonner';

interface OrganizationTagsSelectorProps {
...
export function OrganizationTagsSelector({ 
  selectedTagIds, 
  onTagsChange,
  className 
}: OrganizationTagsSelectorProps) {
  ...
  const [tagToDelete, setTagToDelete] = useState<OrganizationTag | null>(null);
  
  const { data: allTags = [], isLoading } = useOrganizationTags();
  const createTagMutation = useCreateOrganizationTag();
  const updateTagMutation = useUpdateOrganizationTag();
  const deleteTagMutation = useDeleteOrganizationTag();
  ...
  const handleStartEdit = (tag: OrganizationTag) => {
    setEditingTag(tag);
  ...
  return (
    <>
      ...
      <DeleteConfirmDialog
        ...
        description={`Tem certeza que deseja excluir a etiqueta "${tagToDelete?.name}"? Esta ação removerá a etiqueta de todas as organizações.`}
